...
class RequestHandler : KoinComponent {
    ...
    fun handle(request: Request): Pair<Command, Response> {
        ...
        return if (request.requestType == RequestType.SCRIPT && commandToProcess is Autogeneratable) {
            Pair(commandToProcess, commandToProcess.execute(commandArguments?.let { "$it --auto" } ?: "--auto"))
        } else {
            Pair(commandToProcess, commandToProcess.execute(commandArguments))
        }
    }

    fun handleWithObject(element: Vehicle, commandToProcess: Autogeneratable, index: Int): Response {
        return commandToProcess.executeWithObject(element, index)
    }
